...
class TlsSniSocketFactory : SSLSocketFactory {
    ...
    override fun createSocket(
        ...
        i(TAG, "customized createSocket. host: $peerHost")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            i(TAG, "Setting SNI hostname")
            sslSocketFactory.setHostname(ssl, peerHost)
        } else {
            d(TAG, "No documented SNI support on Android <4.2, trying with reflection")
            try {
                val setHostnameMethod = ssl.javaClass.getMethod("setHostname", String::class.java)
                setHostnameMethod.invoke(ssl, peerHost)
            } catch (e: Exception) {
                e(TAG, e, "SNI not useable")
            }
        }
