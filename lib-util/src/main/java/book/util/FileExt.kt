...
fun Uri.getFileFromDocuments(): File? {
    grantPermissions(AppGlobal.application!!)
    val uriId = when {
        DocumentsContract.isDocumentUri(AppGlobal.application, this) -> {
    ...
    when {
        ...
        split[1].contains("Android/data/${AppGlobal.application!!.packageName}") -> {
...
fun File?.saveToAlbum(name: String? = null): Boolean {
    ...
    runCatching {
        ...
        val resolver = AppGlobal.application!!.contentResolver
 ...
 * @return uri
 */
fun Context.createUriInPublicDir(
    name: String,
    dir: String = Environment.DIRECTORY_DOCUMENTS
): Uri? {
...
fun String?.isScopeFile(): Boolean {
    ...
    this ?: return false
    //内部存储
    val filesDirString = AppGlobal.application!!.filesDir.parent
    ...
    //外部存储
    val externalFilesDirString = AppGlobal.application!!.getExternalFilesDir(null)?.parent
...
fun Uri?.isFileExists(): Boolean {
    ...
    return try {
        afd = AppGlobal.application!!.contentResolver.openAssetFileDescriptor(this, "r")
        afd != null
...
fun Uri.getMimeTypeByUri(): String? {
    ...
    return AppGlobal.application?.contentResolver?.getType(this)
}
